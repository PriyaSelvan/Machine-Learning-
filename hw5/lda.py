...
if __name__ == "__main__":
    ...
    flags.documents = 'news.txt'
    flags.vocab = 'news.vob'
    ...
    labs = numpy.ndarray.tolist(unique_labels)

   
    translator = str.maketrans('','',string.punctuation)
    documents = open('news.txt',"w")
    vocabulary = open('news.vob',"w")
    vocab = set()
    with open('../data/20newsgroup.json', 'r') as f:
        text = f.read()
        dataset = json.loads(text)
        ...
        for doc in text:
            lowers = doc.lower()
            no_punct = lowers.translate(translator)
            tokens = nltk.word_tokenize(no_punct)
            filtered = [w for w in tokens if not w in stopwords.words('english')]
            #print (filtered)
            vocab.update(set(filtered))
            for word in filtered:
                documents.write(word+" ")
            documents.write("\n")
    for word in vocab:
    ...
    gamma_sums = numpy.zeros((20,20))
    for docu in range(0,len(text)):
        label_docu = labs.index(labels[docu])     
        gamma_sums[label_docu] += vb._gamma[docu]
    float_formatter = lambda x: "%.2f" % x

    for i in range(0,20):
        print ("for cluster "+labs[i]+":")
        print(numpy.argmax(gamma_sums[i]))
        print(float_formatter(max(gamma_sums[i])))
        print ('\n')
